...
/**
 * Use Codex 5.3 for extraction. If unavailable or missing credentials, return null.
 */
async function selectExtractionModel(
	...
	const model = modelRegistry.find(CODEX_PROVIDER, CODEX_MODEL_ID);
	if (!model) {
		return null;
	}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			const extractionModelSelection = await selectExtractionModel(ctx.modelRegistry);
			if (!extractionModelSelection) {
				ctx.ui.notify(`Model ${CODEX_MODEL_ID} (provider ${CODEX_PROVIDER}) is unavailable or missing API key`, "error");
				return;
			}
			const { model: extractionModel, apiKey: extractionApiKey } = extractionModelSelection;
			...
			const extractionResult = await ctx.ui.custom<ExtractionResult | null>((tui, theme, _kb, done) => {
				...
				const doExtract = async () => {
					...
					const response = await complete(
						...
						{ apiKey: extractionApiKey, signal: loader.signal },
					);
